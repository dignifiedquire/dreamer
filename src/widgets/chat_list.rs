...
pub fn render(ui: &mut Ui, state: &AppState) {
    SidePanel::right("chatlist")
        .frame(Frame::default())
        .min_width(ui.available_width().max(200.))
        .resizable(false)
        .show_inside(ui, |ui| {
            ui.with_layout(
                    ...
                    ScrollArea::vertical()
                        ...
                        .show(ui, |ui| {
                            ...
                            for (i, chat) in chats.chats.iter().enumerate() {
                                let bg_color = if Some(chat.id)
                                    == shared_state.shared_state.selected_chat_id
                                {
    ...
    let response = ui
        .add_enabled_ui(true, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    ui.label(
                        RichText::new(truncate(&chat.name, 50).as_ref())
                            .family(egui::FontFamily::Name(FONT_SEMI_BOLD.into()))
                            .size(18.),
                    );

                    ui.label(
                        RichText::new(truncate(&chat.preview, 70).as_ref())
